...
from ultralytics.data.m_heatmap_utils import ensure_m2d
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        mosaic_labels = []
        s = self.imgsz
        any_m = ("m_heatmap" in labels) or any("m_heatmap" in lb for lb in labels.get("mix_labels", []))
        m3 = None
        for i in range(3):
            ...
            if i == 0:  # center
                ...
                if any_m:
                    m3 = np.zeros((s * 3, s * 3), dtype=np.float32)
                h0, w0 = h, w
            ...
            if any_m:
                m3[y1:y2, x1:x2] = hm[y1 - padh :, x1 - padw :]
        ...
        if any_m:
            final_labels["m_heatmap"] = m3[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                if any_m:
                    m4 = np.zeros((s * 2, s * 2), dtype=np.float32)
            ...
            if any_m:
                m4[y1a:y2a, x1a:x2a] = hm[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                ...
                if any_m:
                    m9 = np.zeros((s * 3, s * 3), dtype=np.float32)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            ...
            if any_m:
                m9[y1:y2, x1:x2] = hm[y1 - padh :, x1 - padw :]
            hp, wp = h, w  # height, width previous for next iteration
        ...
        if any_m:
            final_labels["m_heatmap"] = m9[-self.border[0] : self.border[0], -self.border[1] : self.border[1]]
        return final_labels
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if "m_heatmap" in labels:
            m1 = ensure_m2d(labels["m_heatmap"])
            m2 = labels2.get("m_heatmap")
            if m2 is None:
                m2 = np.zeros_like(m1, dtype=np.float32)
            else:
                m2 = ensure_m2d(m2)
                if m2.shape != m1.shape:
                    m2 = cv2.resize(m2, (m1.shape[1], m1.shape[0]), interpolation=cv2.INTER_LINEAR)
            labels["m_heatmap"] = (m1 * float(r) + m2 * float(1 - r)).astype(np.float32)
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if "m_heatmap" in labels:
            m = ensure_m2d(labels["m_heatmap"])
            chg = (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any()
            if chg:
                if self.perspective:
                    m = cv2.warpPerspective(m, M, dsize=self.size, borderValue=0.0)
                else:
                    m = cv2.warpAffine(m, M[:2], dsize=self.size, borderValue=0.0)
            labels["m_heatmap"] = m.astype(np.float32)
        return labels
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            ...
            instances.flipud(h)
            if "m_heatmap" in labels:
                labels["m_heatmap"] = np.flipud(ensure_m2d(labels["m_heatmap"]))
        if self.direction == "horizontal" and random.random() < self.p:
            ...
            if "m_heatmap" in labels:
                labels["m_heatmap"] = np.fliplr(ensure_m2d(labels["m_heatmap"]))
        ...
        if len(labels):
            if "m_heatmap" in labels:
                ...
                if shape[::-1] != new_unpad:
                    shrink = m.shape[0] > new_unpad[1] or m.shape[1] > new_unpad[0]
                    m = cv2.resize(m, new_unpad, interpolation=cv2.INTER_AREA if shrink else cv2.INTER_LINEAR)
                m = cv2.copyMakeBorder(m, top, bottom, left, right, cv2.BORDER_CONSTANT, value=0.0)
                labels["m_heatmap"] = m.astype(np.float32)
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1, labels2={}):
        ...
        if "m_heatmap" in labels1:
            m_out = ensure_m2d(labels1["m_heatmap"]).copy()
            if "img" in labels2:
                m_res = labels2.get("m_heatmap")
                if m_res is None:
                    m_res = np.zeros((result.shape[0], result.shape[1]), dtype=np.float32)
                else:
                    m_res = ensure_m2d(m_res)
            else:
                m_res = np.fliplr(ensure_m2d(labels1["m_heatmap"]))
            if m_res.shape[0] != result.shape[0] or m_res.shape[1] != result.shape[1]:
                m_res = cv2.resize(m_res, (result.shape[1], result.shape[0]), interpolation=cv2.INTER_LINEAR)
            m_out[i] = m_res[i]
            labels1["m_heatmap"] = m_out.astype(np.float32)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        img = labels.pop("img")
        h, w = img.shape[:2]
        m_heatmap = labels.pop("m_heatmap", None)
        cls = labels.pop("cls")
        instances = labels.pop("instances")
        ...
        if m_heatmap is not None:
            if isinstance(m_heatmap, np.ndarray):
                mh = torch.from_numpy(ensure_m2d(m_heatmap).astype(np.float32)).unsqueeze(0)
            else:
                mh = m_heatmap.float()
                if mh.ndim == 2:
                    mh = mh.unsqueeze(0)
            labels["m_heatmap"] = mh.contiguous()
