...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        channels = model.yaml.get("ch", 3) if isinstance(getattr(model, "yaml", None), dict) else 3
        if not isinstance(imgsz, list):
            imgsz = [imgsz, imgsz]  # expand if int/float
        try:
            # Use stride size for input tensor
            stride = max(int(model.stride.max()), 32) if hasattr(model, "stride") else 32  # max stride
            im = torch.empty((1, channels, stride, stride), device=p.device, dtype=p.dtype)
            flops = thop.profile(deepcopy(model), inputs=[im], verbose=False)[0] / 1e9 * 2  # stride GFLOPs
            return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs
        except Exception:
            # Use actual image size for input tensor (i.e. required for RTDETR models)
            im = torch.empty((1, channels, *imgsz), device=p.device, dtype=p.dtype)
